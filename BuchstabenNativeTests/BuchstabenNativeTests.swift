...
final class BuchstabenNativeTests: XCTestCase {
    ...
    func testMultiTouchNavigationClearsAndSuppressesSingleTouchBriefly() {
        let vm = TracingViewModel(singleTouchCooldownAfterNavigation: 0.05, haptics: StubHaptics())
    ...
    func testTracingViewModelUsesInjectedAudioControllerAcrossLifecycle() {
        ...
        let vm = TracingViewModel(singleTouchCooldownAfterNavigation: 0, audio: audio, haptics: StubHaptics())
    ...
    func testRepeatedBeginMultiTouchDoesNotLeaveStuckState() {
        let vm = TracingViewModel(singleTouchCooldownAfterNavigation: 0, haptics: StubHaptics())
    ...
    func testTouchBurstInterruptedByMultiTouchNav_suppressionApplied() {
        let audio = MockAudioController()
        let vm = TracingViewModel(singleTouchCooldownAfterNavigation: 0.05, audio: audio, haptics: StubHaptics())
        let size = CGSize(width: 320, height: 480)
