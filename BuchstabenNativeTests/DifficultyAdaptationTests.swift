...
@Suite struct MovingAverageAdaptationPolicyTests {
    ...
    @Test func emptyHistory_nonStandardInitialTier_keepsTierAndZeroAccuracy() {
        let policy = makePolicy(initial: .easy)
        #expect(policy.currentTier == .easy)
        #expect(policy.samples.isEmpty)
        #expect(abs(policy.windowAccuracy) < 1e-9)
    }

    @Test func promotion_thenDemotion_updatesCurrentTierAcrossMultipleRecords() {
    ...
    @Test func demotion_thenPromotion_updatesCurrentTierAcrossMultipleRecords() {
        var policy = makePolicy(
            windowSize: 2,
            hysteresisCount: 2,
            promotionThreshold: 0.85,
