...
| **W-17** | 🟡 Warning | **FIXED** | ProgressStore + ParentDashboardStore + StreakStore | `schemaVersion` sentinel on every persisted root struct; load() refuses to read a forward-incompatible file and logs warning |
| **W-24** | 🟡 Warning | **FIXED** | LetterScheduler:166 | `effectiveStabilityDays(for:)` stretches per-letter Ebbinghaus stability with completion count + accuracy, capped at 60 days |
| **I-2** | 🟢 Info | **FIXED** | TracingDependencies:16 | Doc comment now spells out the `nil → FixedAdaptationPolicy on .control, MovingAverage otherwise` fallback so the dependency intent reads from the type itself |
...
| # | Risk | Impact | Status |
|---|------|--------|--------|
| D-1 | `schedulerEffectivenessProxy` always 0 | (replaced by C-3) | **FIXED** (C-3) |
| D-2 | `recognitionSamples` is a 10-deep rolling per-letter window with no per-sample timestamps. The CSV exporter could only attach "the latest sample" to every phase row, mis-correlating recognition with earlier phases. | Per-session recognition recoverable from CSV again. | **FIXED** — `PhaseSessionRecord` now carries `recognitionPredicted/Confidence/Correct` populated at session-completion time; the W-2 blank-column workaround is reversed for new rows. |
| D-3 | `PhaseSessionRecord` lacked a timestamp, so dated learning curves and time-windowed analyses were impossible from the CSV. | Dated learning curves now exportable. | **FIXED** — `recordedAt: Date` field on `PhaseSessionRecord`, ISO-8601 in CSV, decoder default-nil for legacy files. |
| D-4 | The `.guidedOnly` and `.control` arms only run the `.guided` phase, so they never produce freeWrite data. That's where Schreibmotorik dimensions and the freeform recognition path live, so those columns are structurally empty for two of the three arms. | Between-condition ANOVA has empty cells | **By design — truly deferred.** This is the experimental contrast itself. Comparing freeWrite metrics between arms is invalid by definition; the thesis methodology must lean on metrics that are produced in every arm (session count, guided-phase accuracy, recognition results in freeform mode if enabled, etc.). To be acknowledged in the limitations chapter, not patched in code. |
| D-5 | Session duration is computed as `endedAt − startedAt`. The clock keeps running when the app is backgrounded, so a "4-minute" session might include 3 minutes of the iPad sitting on a table. | Duration analysis unreliable. | Open. Switching to active-time (pause on `scenePhase != .active`) needs reconciliation across `FreeWritePhaseRecorder`, `ParentDashboardStore`, and `JSONProgressStore`. Tractable but larger than the round-3 cleanup pass; queued for a follow-up. |
| D-6 | `speedTrend` (the rolling per-letter writing-speed series used by the scheduler's `automatizationBonus`) was JSON-only. | Automatization analysis available from CSV. | **FIXED** — `speedTrend` semicolon-joined into the per-letter row in CSV/TSV. |
| D-7 | Pre-enrolment activity (pilot taps, parent demos, sandbox sessions) decoded as `.threePhase` and was indistinguishable from real `.threePhase` arm data. | Pilot-contamination risk eliminated at export time. | **FIXED** — `ParticipantStore.enrolledAt` is stamped when `isEnrolled` flips on; the CSV exporter discards rows older than this date and surfaces the cutoff via a `# enrolledAt=` header line. |
